...
- [ ] **P0-1 — Request Coston2 indexer credentials.** Via `https://flare.network/resources/technical-support` or `@FlareDevs`, stating what is being built. Coston2 indexer is `34.38.42.208:3306`, database `indexer`, no VPN. Do not target Coston — different credentials, VPN required. *This is the first action of the project.*
- [ ] **P0-2 — Fund the Coston2 wallet.** `https://faucet.flare.network/coston2`. Needs headroom for gas, extension registration, and per-instruction fees, not a single transaction's worth.
- [ ] **P0-3 — Clone and pin the scaffold.** `flare-foundation/fce-extension-scaffold` at a fixed commit. Record the commit hash in the README. FCC is not a fully public production system; an unpinned scaffold is an API change waiting to break the build.
- [ ] **P0-4 — Reserve an ngrok domain.** Free tier gives one reserved domain that survives restarts. cloudflared quick tunnels mint a new URL per run and force an `EXT_PROXY_URL` edit plus re-registration each time.
- [ ] **P0-5 — Fill `.env`.** `DEPLOYMENT_PRIVATE_KEY`, `INITIAL_OWNER`, `PROXY_PRIVATE_KEY`, `CHAIN_URL`, `ADDRESSES_FILE`, `LOCAL_MODE=false`, `SIMULATED_TEE=true`, `NORMAL_PROXY_URL`, `EXT_PROXY_URL`. `SIMULATED_TEE=true` must pair with container `MODE=1` or registration fails `code hashes do not match`.
- [ ] **P0-6 — Fill the indexer `[db]` block** in `config/proxy/extension_proxy.coston2.docker.toml` with the issued credentials.
- [ ] **P0-7 — Run the scaffold unmodified.** `ngrok http 6674` → `pre-build.sh` → `start-services.sh --chain coston2` → `post-build.sh` → `test.sh`. `SAY_HELLO` and `SAY_GOODBYE` both complete.
- [ ] **P0-8 — Record the environment.** Extension id, instruction sender address, code hash, tunnel domain, scaffold commit — written down where the team can find them, because a reset means reproducing them exactly.
