...
- [!] **P0-1 — Request Coston2 indexer credentials.** Via `https://flare.network/resources/technical-support` or `@FlareDevs`, stating what is being built. Coston2 indexer is `34.38.42.208:3306`, database `indexer` — **verified open without a VPN**. Do not target Coston: different credentials, and `35.241.249.150:3306` is filtered from a plain connection. *This is the first action of the project.* Request text drafted in `phase-0-status.md`.
...
- [~] **P0-4 — Reserve an ngrok domain.** Binary installed (3.39.10). Account, `ngrok config add-authtoken`, and the reserved domain are a human step. Free tier gives one reserved domain that survives restarts; cloudflared quick tunnels mint a new URL per run and force an `EXT_PROXY_URL` edit plus re-registration each time.
...
- [~] **P0-6 — Fill the indexer `[db]` block.** `config/proxy/extension_proxy.coston2.docker.toml` and `.coston2.toml` created from their examples with host, port and database filled in and verified reachable. `username` and `password` pending P0-1. Both files are gitignored.
- [!] **P0-7 — Run the scaffold unmodified.** `ngrok http 6674` → `pre-build.sh` → `start-services.sh --chain coston2` → `post-build.sh` → `test.sh`. `SAY_HELLO` and `SAY_GOODBYE` both complete. Blocked on P0-1, P0-2, P0-4. Everything short of the funded transaction is verified: bindings generate, `forge build` is clean, the Go extension builds/vets/tests under Go 1.26.4, version pins are consistent, the `aegis-extension-tee` image builds, and pre-flight reaches Coston2 and stops only on balance.
