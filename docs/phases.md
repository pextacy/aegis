...
| 1 | Policy layer in Solidity | — | `[x]` 199 contract tests |
...
| 6 | Multisig and PMW migration | 5 | `[~]` k-of-n built and proven across three real enclaves; PMW swap and key import stay post-program |

**Phase 6's first three tasks are done, and that changes what v1's headline limitation says.** Aegis no longer only *plans* k-of-n: a treasury can commit to k of n, collect a signer key from each of n enclaves, hand its XRPL account to them with a `SignerList`, retire the master key that could have ignored them, and pay by quorum thereafter. `./scripts/local-quorum.sh` runs all of it against three separate enclave processes on a local chain, and the multi-signing digest is validated against signatures other software produced for a transaction XRPL validated. What is still post-program is the PMW swap and the key-import path, which are P6-4 and P6-5.
...
**And `toProduction` is not Aegis code.** `grep -rn toProduction contracts/ script/` returns nothing; it lives in `tools/pkg/fccutils/registration.go`, which is the vendored scaffold's deployment tooling. No Aegis contract calls it, and none ever will — it is a step an operator runs once per machine. The same is true of `FdcHub`: no contract in `contracts/` requests an attestation, because requesting one is the submitter's job and verifying one is `ExecutionVerifier`'s, and the verifying half is checked against Flare's real verifier.

**And the root substitution is gone too.** The FDC verifier API key turns out to gate only `prepareRequest`. Everything downstream is public: attestation requests are on-chain `AttestationRequest` events on `FdcHub`, the DA layer serves proofs without authentication, and finalised roots sit in `Relay`. So a real request that somebody else paid for can be read off the chain, its proof fetched, and the whole thing checked against the root the network actually finalised.

`test/fork/RealAttestationFork.t.sol` pins one: a real `Payment` attestation on `testXRP` — the exact attestation type and source id Aegis uses — requested in block 33546268 and finalised in round 1413928. It decodes as our `IPayment.Response` (so the struct has not drifted from Flare's), the round is confirmed finalised on chain, the real `FdcVerification` accepts it against the real `Relay` root with **nothing etched, stored or stubbed**, and it is still rejected once a single field is altered. That last assertion is what stops the others from being vacuous.

A survey of 168 recent attestation requests also settles a question worth having asked: `Payment / testXRP` is the second most common type on Coston2 and `ReferencedPaymentNonexistence / testXRP` the third, which are exactly the two Aegis consumes. The constants are right.

**And that last call runs too.** The FDC verifier API key is published for testnet: `00000000-0000-0000-0000-000000000000`. `submitter/test/live-verifier.test.ts` calls the real `prepareRequest` against `fdc-verifiers-testnet.flare.network` with our own `paymentRequestBody` and `nonexistenceRequestBody`, for the real XRPL transaction this system settled, and checks the returned `abiEncodedRequest` opens with the attestation type and source id we asked for — a mis-encoded name would be refused by the server and never by a stub. It also asserts a wrong key raises `FdcError` rather than returning something the submitter would carry forward. Nothing in `submitter/src` defaults to that key; `FDC_VERIFIER_API_KEY` is still required, because a mainnet deployment needs one genuinely issued and a default would be the silent fallback this codebase refuses everywhere else.

That closes the settlement path end to end. Its arithmetic is not taken on trust either — `submitter/test/live-fdc.test.ts` reads the voting-round parameters from the real `FlareSystemsManager`, derives the round the submitter's way, and compares against `Relay.getVotingRoundId` for the same timestamps. Two independent derivations with no expected value written by hand, across 24 samples including round boundaries. A wrong answer there would request attestations for a round whose proof never appears, and every payment would stall in `Signed` looking like Flare being slow rather than like a bug here.
...
**The FDC verifier's API key looked like a second blocker and is not one on testnet.** `https://fdc-verifiers-testnet.flare.network` answers `401 Unauthorized` without a key, which is what made it look gating; Flare publishes `00000000-0000-0000-0000-000000000000` for its testnet verifiers, and with that the real `prepareRequest` runs. Only `prepareRequest` was ever gated at all — the DA layer answers a domain error rather than a 401, so proof retrieval needed nothing. A mainnet deployment does need a key that is genuinely issued.
...
- [x] **P1-11b — `PaymentController.sol`: `revokeApproval`.** Not in the original plan, and the timelock does not mean much without it. A tier's timelock exists so that an approver has time to reconsider, but until now the only way to act on reconsidering was to ask the proposer to cancel or to have a guardian freeze the treasury — which stops every payment in flight rather than the one being objected to. An approver withdraws their own approval while the request is `Proposed` or `Approved`; the count drops and an `Approved` request returns to `Proposed`, so a dispatch cannot proceed on authorisation that no longer holds. Three deliberate asymmetries with `approve`, each of which would be a bug the other way round: no role check, because an address whose `ROLE_APPROVER` was removed still occupies a slot in the stored count and the approver set — P1-13b stops that slot carrying weight at dispatch, but only its owner can clear it; no freeze check, because revoking is a withdrawal of authorisation and a freeze that preserved approvals would protect the payment it interrupted; and only the approver themselves, because anyone else striking a third party's approval would be a veto no role grants. `eligibleAt` is untouched — restarting the clock would let one approver postpone a payment indefinitely by approving and withdrawing.
- [x] **P1-13b — `dispatch` re-checks *who* approved, not just how many.** Also not in the original plan, and it is the same class of bug as P1-11b rather than a coincidence: both come from an approval being treated as a fact once recorded rather than as authority borrowed from a role. `dispatch` re-ran the price, the allowlist, the tier, the window, the freeze and the sequence, and then compared a stored *count* against the threshold. So a policy admin who revoked an approver's role — which is what you do the moment you learn an approver is compromised — did not affect any request that approver had already signed off. Their vote still carried the payment. The contract's own header comment calls this out as the thing dispatch exists to prevent: "a request that was valid an hour ago is not automatically valid now — grandfathering is how treasuries get drained." The approver set was the one input still being grandfathered. Fixed by storing the approvers rather than a tally, and counting at dispatch only those who still hold `ROLE_APPROVER`. The check reads current authority rather than recording a strike, so restoring the role restores the payment, and a lapsed approval only blocks a dispatch when it takes the request under the threshold that applies *at dispatch* — which is re-resolved from the live price and can be lower than the one in force at proposal. `approversOf` and `validApprovals` expose both figures, because a dashboard showing only the raw count would tell an operator a payment is ready when the contract is about to refuse it. The array is bounded by the request's `requiredApprovals`: `approve` refuses any state but `Proposed`, and reaching the threshold is what leaves it.
...
- An approver can withdraw their own approval, and cannot withdraw anyone else's or one never given.
- Withdrawing below the threshold returns the request to `Proposed` and makes a later dispatch revert, even after the timelock has fully elapsed.
- Withdrawing works after the approver role is removed and while the treasury is frozen, and does not restart the timelock.
- Dispatch is refused when an approver's role was revoked after they approved and the remaining valid approvals fall under the threshold, allowed when they still meet it, and allowed again once the role is restored.
- `validApprovals` drops below the stored `approvals` count when a role lapses, and re-approving after a withdrawal leaves no duplicate in the approver set.
...
The visual design is **Lumina Prime**, a Stitch export: a light institutional palette with navy structure and one azure accent, Inter for the interface, JetBrains Mono for every figure and address, and a fixed sidebar. The tokens live in `web/src/app/globals.css`; the shell is `AppShell.tsx` and the primitives are `ui.tsx`. Five things the export drew were deliberately not built. Four because no contract publishes the figures behind them — a treasury health score, a total-value-locked tile with a 24-hour delta, a rolling-24h activity chart, and signer avatars on the payments table. The fifth is the `25% / 50% / MAX` quick-fill on the amount field: XRPL holds back a base reserve, so a MAX derived from the balance proposes an amount the ledger will refuse, and a control that fills in an unspendable number is worse than no control. The live balance it sits beside *is* shown, because that is the part which informs. The stat rows likewise show what the chain and the ledger actually return — XRP held summed across funded accounts, treasury and policy counts, allowlist enforcement, and the block range the log scan reached. Icons are inline SVG rather than the export's Material Symbols webfont, and `next/font` self-hosts Inter and JetBrains Mono, so a page load makes no third-party request: a treasury console that fetched a font from a CDN would leak which addresses are being looked at and when.
...
- [x] **P6-1 — k-of-n across TEE machines.** `TreasuryRegistry.configureSignerSet` commits a treasury to k of n, once; `AegisInstructionSender.requestSignerKeygen` fans one instruction out to n machines via `getRandomTeeIds(extensionId, n)` and each returns a signer key of its own, verified on-chain exactly as the master key is. `PaymentController.dispatch` reads the signing mode at dispatch rather than at proposal — a treasury that moved to k-of-n while a request waited out its timelock must pay by quorum — and `recordPartialSignature` collects shares, checking each key derives to a bound signer and refusing a second from the same one. A share arriving after the quorum closed is recorded as ignored, because a late machine is not a faulty one.
- [x] **P6-2 — XRPL `SignerList`.** `SETUP` signs the `SignerListSet` that hands the treasury's account to its signer set, then the `AccountSet` with `asfDisableMaster` that retires the key which could otherwise have ignored it. Both are master-key signed, so only the machine that ran KEYGEN can answer and the rest refuse for want of a key — the separation the keystore's two maps exist for, and one the local run demonstrates rather than assumes. The order is the only one XRPL permits: retiring first would leave an account nothing could ever sign for.
- [x] **P6-3 — Multi-sign serialisation.** Hash prefix `0x534D5400` with the signer's AccountID appended, per-signer `Signers` array entries, empty `SigningPubKey`. Implemented twice — in the enclave and in the submitter's assembler — and both validated against transactions taken off XRPL rather than against each other.
...
### What was not in the original plan, and had to be

**A second digest.** `SignRequest` carries no source account, because in the single-key path the enclave supplies it from its own keystore. A signer key has no such state — it belongs to the signer, not to the treasury — so the account has to travel with the instruction, and every field that travels with an instruction is one a relayer can alter. Without a check, a legitimately approved payment could be pointed at a different account's signer list. `multiSignDigest = keccak256(abi.encode(policyDigest, sourceAccountId))` binds the two without touching the digest every existing payment is already validated against, and its refusal has its own log line because it names a different tampered field.

**Two keystores rather than one.** A machine can legitimately hold both a master key and a signer key for the same treasury: it created the account and it is also one of the n signers. One map with a role field would make "sign this payment" reachable from the key that gets retired precisely so it cannot sign payments. Two maps make that unreachable rather than merely unintended, and `TestMultiSignWillNotUseTheMasterKey` is the proof.

**The optimizer.** `PaymentController` reached 29,202 bytes with the quorum path added — 4,626 over the deploy limit, so the contract could not have been deployed at all. `optimizer_runs = 200` brings it to 15,491. A low runs count is the right trade: slightly costlier hot paths against a contract that exists.

### What is proven, and how

`./scripts/local-quorum.sh` runs the whole arrangement against a local chain and **three separate enclave processes**, each holding its own key and none holding another's. That is what makes "no single machine can sign alone" a fact about the run rather than a claim about the design. It collects three signer keys, confirms the two signer-only machines refuse to sign a signer list, installs the list, retires the master key, signs a payment with two of three, sees the third arrive late and be ignored, and assembles the shares with the submitter's own code. It ends on the two properties the feature exists for: a payload whose amount was altered is refused by all three with `policy digest mismatch`, and a payload with *only* the source account changed is refused by all three with `source account digest mismatch`.

The serialisation is not taken on trust either. Three vectors are pinned, all fetched from XRPL with `tx` in binary mode: a two-of-n multi-signed `Payment`, the `SignerListSet` that installed the signer list it is validated against, and the `AccountSet` that retired that same account's master key twenty-one ledgers later. Together they are one complete arrangement rather than three unrelated blobs. Reassembling the payment from its decoded fields reproduces XRPL's own bytes and hash in both Go and TypeScript.
...
PMW provides natively what P6-1 to P6-3 now do by hand: wallets as k-of-n key sets across TEE machines on native XRPL and BTC multisig accounts, per-instruction nonces with transaction chaining, reissuance at a different fee, nullification, and FDC execution proofs. Having built them does not make the migration less useful — it makes it cheaper, because the signer set, the quorum accounting and the setup transactions were all built against the same `SignRequest` boundary and can be handed over rather than reconciled. It is also the only route available while system extension ids below `0x10000` remain reserved.
...
- [x] `forge test` and `go test ./...` both pass. 254 contract tests — 238 without a Coston2 RPC, the other 16 being the fork suites — Go suites green, conformance 25/25, 118 submitter tests, 32 dashboard tests.
...
- [x] `opType` / `opCommand` changes touch all three locations. `check.sh` asserts the three-way alignment directly, and since k-of-n added three commands it now checks every one of the six by name rather than only the opType.
...
`PLAN.md` is authoritative on phase numbers: **5 = interface and hardening, 6 = multisig and PMW**. `DOCS.md` §8 said "phase 5" where it meant phase 6, a reference predating the split; that has been corrected at the source now that the section describes something built rather than planned. `CLAUDE.md` still says "phase 5" for key import in passing, and means phase 6.
