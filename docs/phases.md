...
What forking still cannot reach is a TEE machine being registered, which needs a real attestation, and an attestation being finalised in a voting round, which needs the verifier API key. Both are Flare's infrastructure accepting something rather than Aegis code paths.
